...
def run():
    while True:
        ...
        try:
            ...
            start = time.time()
            for tag in Tags:
                Inikeys = db_session.query(SteamEnergy).filter(SteamEnergy.TagClassValue == tag.TagClassValue).order_by(desc("CollectionDate")).all()
                currID = 0
                preID = 0
                icount = 0
                for key in Inikeys:
                    ...
                    if icount == len(Inikeys)-1:
                        ...
                        prekey = db_session.query(SteamEnergy).filter(
                            ...
                            SteamEnergy.CollectionDate < currCollectionDate).order_by(desc("CollectionDate")).first()
                        if prekey != None:
                            preID = prekey.ID
                            ss = (preID,currID)
                            steamInitial.append(ss)
